...
Util:AddValues{
    ...
    ["Table"] = {
        ...
        AllItems = {"Christmas Present", "Mysterious Arrow", "Pure Rokakaka", "Rokakaka", "Diamond", "Lucky Arrow", "Lucky Stone Mask", "Dio's Diary", "Steel Ball", "Rib Cage of The Saint's Corpse", "Stone Mask", "Gold Coin", "Quinton's Glove", "Ancient Scroll", "Zeppeli's Hat", "Caesar's Headband", "Clackers", "Blue Candy", "Yellow Candy", "Red Candy", "Green Candy"};
...
local CreamAnnihilation = {
    Running = false,
    Whitelist = {},  -- {Name = "xxx", UserId = 12345}
    PullLoop = nil,
    SpamLoop = nil
}
...
local function IsWhitelisted(plr)
    for _, entry in ipairs(CreamAnnihilation.Whitelist) do
...
local function AddToWhitelist(selected)
    local name = selected:match("^(.-) %(")
    if not name then name = selected end
    local plr = Players:FindFirstChild(name)
    if plr and not IsWhitelisted(plr) then
        table.insert(CreamAnnihilation.Whitelist, {Name = plr.Name, UserId = plr.UserId})
        Library:Notification("Cream Whitelist", plr.Name .. " has been added to whitelist", 3)
    end
end
...
local function GetPlayerList()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            table.insert(list, plr.Name .. " (" .. plr.UserId .. ")")
        end
    end
    table.sort(list)
    return list
...
-- Skill spam (proven working method)
local function StartSpam()
    CreamAnnihilation.SpamLoop = task.spawn(function()
        while CreamAnnihilation.Running do
            task.wait()
            pcall(function()
                local char = Util:GetCharacter()
                if char and char:FindFirstChild("RemoteEvent") then
                    local re = char.RemoteEvent
                    re:FireServer("InputBegan", {Input = Enum.KeyCode.T})  -- Dark Space
                    re:FireServer("InputBegan", {Input = Enum.KeyCode.Y})  -- Devastation
                    re:FireServer("InputBegan", {Input = Enum.KeyCode.X})  -- Madness Sorrow
                    re:FireServer("InputBegan", {Input = Enum.KeyCode.G})  -- Hamon Charge (optional)
                end
            end)
        end
    end)
...
-- Pull all non-whitelisted players
local function StartPull()
    CreamAnnihilation.PullLoop = task.spawn(function()
        while CreamAnnihilation.Running do
            task.wait(0.05)
            local myRoot = Util:GetHRP()
            if not myRoot then continue end

            for _, plr in ipairs(Players:GetPlayers()) do
                if plr == LocalPlayer or IsWhitelisted(plr) then continue end
...
-- GUI Section
local CreamSec = Misc:Section("Cream Annihilation", "Left")

local MainToggle = CreamSec:Toggle("Cream Annihilation", false, function(state)
    CreamAnnihilation.Running = state

    if state then
        if not LocalPlayer.PlayerStats.Stand.Value:find("Cream") then
            Library:Notification("Error", "Cream stand is required", 5)
            MainToggle:UpdateAsset(false)
            return
        end
        StartSpam()
        StartPull()
        Library:Notification("Cream Annihilation", "Enabled – all non-whitelisted players are affected", 4)
    else
        if CreamAnnihilation.SpamLoop then task.cancel(CreamAnnihilation.SpamLoop) end
        if CreamAnnihilation.PullLoop then task.cancel(CreamAnnihilation.PullLoop) end
        Library:Notification("Cream Annihilation", "Disabled", 3)
    end
end)

CreamSec:Label("Whitelist – selected players will not be affected")

-- Dropdown populated immediately
local PlayerDropdown = CreamSec:Dropdown("Select Player ", GetPlayerList(), function(choice)
    AddToWhitelist(choice)
end)
...
CreamSec:Button("Refresh Player List", function()
    PlayerDropdown:Refresh(GetPlayerList())
end)

CreamSec:Button("Clear Whitelist", function()
    CreamAnnihilation.Whitelist = {}
    Library:Notification("Whitelist cleared", "", 3)
end)

local WhitelistLabel = CreamSec:Label("Whitelist: empty")
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        WhitelistLabel:UpdateAsset("Whitelist: " .. (#names == 0 and "empty" or table.concat(names, ", ")))
    end
end)
